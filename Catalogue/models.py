...
class Record(models.Model):
    catalogue_num = models.IntegerField(default=1, primary_key=True) #default is what is registered as value if no other value is received
    name = models.CharField(max_length=100) #string that has a max length of 100 characters
    date = models.CharField(max_length=100, blank=True, null=True) #up to 100 characters, so it can give a description or range of dates
...
class Sculpture(models.Model):
    record = models.OneToOneField(Record, on_delete=models.CASCADE)
    material = models.CharField(max_length=100, blank=True, null=True) #material used to build sculpture
...
class Coin(models.Model):
    ...
    design = models.TextField(blank=True, null=True) # what object or person (user able to put extra information about the person) is on the coin if any
...
class Artwork(models.Model):
    ...
    supplies_used = models.CharField(max_length=100, blank=True, null=True) #paintbrush, pencil, etc
...
class Medal(models.Model):
    record = models.OneToOneField(Record, on_delete=models.CASCADE)
    medal_type = models.CharField(max_length=100, blank=True, null=True) #what was this medal awarded for
    colour = models.CharField(max_length=100, blank=True, null=True)
    material = models.CharField(max_length=100, blank=True, null=True)
...
class Weapon(models.Model):
    ...
    combat = models.CharField(max_length=100, blank=True, null=True) #is this weapon long distance or close combat
    size = models.IntegerField(blank=True, null=True)
    weight = models.IntegerField(blank=True, null=True)
    material = models.CharField(max_length=100, blank=True, null=True)
...
class Book(models.Model):
    record = models.OneToOneField(Record, on_delete=models.CASCADE)
    author = models.TextField(blank=True, null=True) #it is text field in case there are many authors
